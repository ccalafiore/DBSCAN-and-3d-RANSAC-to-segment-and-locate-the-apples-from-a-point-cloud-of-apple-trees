...
def load_points(dir_project, with_trees=True, max_colors_1=False, delimiter=','):
    ...
    dir_parts = os.path.join(dir_data, 'apple_trees')
    ...
    if with_trees:
        ...
        #     dir_part_i = os.path.join(dir_parts, 'apple_trees_part_{:0>2d}.txt'.format(i))
        #     cp.txt.lines_to_csv_file(lines=points_parts[i], directory=dir_part_i, headers=None, delimiter=' ')

        dirs_parts = glob.glob(os.path.join(dir_parts, 'apple_trees_part_*.txt'))
